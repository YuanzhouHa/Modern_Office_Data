...
def spawn_actor(asset_path, location=unreal.Vector(0.0, 0.0, 0.0), random_rotation=False):
    # spawn actor into level
    obj = unreal.load_asset(asset_path)
    
    # 设置旋转值
    if random_rotation:
        # 随机水平旋转角度(yaw)，保持pitch和roll为0以确保角色直立
        random_yaw = random.uniform(0, 360)
        rotation = unreal.Rotator(random_yaw, 0, 0)
    else:
...
if __name__ == '__main__':
    ...
    actor = spawn_actor(asset_path=selected_skeletal_mesh_path, location=location, random_rotation=True)
    spawnable_actor = level_sequence.add_spawnable_from_instance(actor)
    add_animation_to_actor(spawnable_actor, animation_path=selected_animation_path)
    ...
    bind_camera_to_level_sequence(level_sequence, camera, start_frame=0, num_frames=150, move_radius=100)
