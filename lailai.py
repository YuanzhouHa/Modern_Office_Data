...
def find_relevant_assets():
    ...
    for actor in all_actors:
        if 'HDRIBackdrop' in actor.get_name():
            hdri_backdrop = actor
        if 'SuperCine' in actor.get_name():
            camera = actor
...
def render(output_path, start_frame=0, num_frames=0, mode="rgb"):
    ...
    job.set_editor_property('sequence', unreal.SoftObjectPath('/Game/test'))
...
if __name__ == '__main__':
    ...
    location = unreal.Vector(0.0, 100.0, 0.0)
