...
def render(output_path, start_frame=0, num_frames=0, mode="rgb"):
    ...
    job.set_editor_property('sequence', unreal.SoftObjectPath('/Game/RenderSequencer_Alpha'))
    ...
    def movie_finished(pipeline_executor, success):
        ...
        if mode == 'rgb':
            render(output_path=output_path, mode="alpha")
        # elif mode == 'normals':
...
if __name__ == '__main__':
    ...
    level_sequence = unreal.load_asset('/Game/RenderSequencer_Alpha')
    ...
    selected_skeletal_mesh_path = select_random_asset('/Game/CC4_Character/', asset_class='SkeletalMesh')
    ...
    bind_camera_to_level_sequence(level_sequence, camera, start_frame=0, num_frames=150, move_radius=300)
    ...
    render(output_path="D:\\SyntheticData\\rgb_alpha\\" + timestamp + "\\", mode="rgb")